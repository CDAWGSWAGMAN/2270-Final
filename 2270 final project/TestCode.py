...
class Hand:
    def __init__(self):
        ...
        self.winner = None
        self.font = pygame.font.Font(GAME_FONT, 60)
        ...
        self.current_round = 0  # 0 for pre-flop, 1 for post-flop, 2 for turn, 3 for river
        self.active_players = self.player_list.copy()
    ...
    def handle_betting_round(self):
        # Logic for handling player actions during a betting round
        pass  # Placeholder for betting logic
    ...
    def render_winner(self):
        if self.dealer.determined_winner is not None:
            ...
            elif self.dealer.determined_winner == "Player 3":
                text = "Player 3 Wins!"
                text_color = (115, 235, 0)  # Blue
            # elif self.dealer.determined_winner == "Player 4":
            #     text = "Player 4 Wins!"
            #     text_color = (135, 206, 235)  # Green
...
class Dealer():
    def __init__(self, players, flop):
        ...
        self.printed_add_card_to_flop = False
    ...
    def add_card_to_flop(self):
        if self.can_deal and self.can_deal_flop and self.dealt_cards - (self.num_players * 2) < 5:
            #self.card_audio()
            self.flop.cards.append(self.deck[-1])
            flop_x = self.players_list[0].cards[0].card_surf.get_width() * 2 + (self.players_list[0].cards[0].card_surf.get_width() * self.current_flop_index * 2 + 40)
            self.flop.cards[-1].position = (flop_x, self.flop.cards[-1].card_y)
            self.deck.pop(-1)
            self.last_dealt_flop_time = pygame.time.get_ticks()
            self.can_deal_flop = False
